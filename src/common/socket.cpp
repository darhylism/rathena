...
void gepard_session_unit(unsigned int seed, struct gepard_crypt_unit* unit)
{
	...
	for (i = 0; i < 256; ++i)
	{
		unit->key[i] = (((seed -= seed * SESSION_CONST_1 - SESSION_CONST_2 * i) >> 14) & 0xFF);
	}
...
void gepard_enc_dec(unsigned char* data, uint32 data_size, struct gepard_crypt_unit* unit)
{
	...
	for(i = 0; i < data_size; ++i)
	{
		...
		unit->pos_2 += unit->key[unit->pos_1] * ALGO_KEY_1;
		unit->pos_1 += unit->key[unit->pos_2] * ALGO_KEY_2;
		unit->key[unit->pos_2] ^= unit->pos_3;
		unit->pos_1 += unit->pos_3 ^ ALGO_KEY_3;
